...
{
    return [UIColor colorWithHex:0xf67d02];//colorWithHex:0x15A230
}
...
+ (UIColor *)colorFromHexRGB:(NSString *)inColorString alpha:(CGFloat)alpha {
    inColorString = [[inColorString stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]] uppercaseString];
    // String should be 6 or 8 characters
    
    if ([inColorString length] < 6) return [UIColor blackColor];
    // strip 0X if it appears
    if ([inColorString hasPrefix:@"0X"]) inColorString = [inColorString substringFromIndex:2];
